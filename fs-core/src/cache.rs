    ...
    fn from(err: CacheError) -> Self {
        match err {
            ...
            CacheError::CacheFull => {
                BlockDeviceError::Io(io::Error::new(io::ErrorKind::OutOfMemory, "Cache is full"))
            }
    ...
    pub async fn write_block(&self, block_num: u64, data: &[u8]) -> Result<()> {
        ...
        if let Some((evicted_block_num, evicted)) = maybe_evicted {
            if evicted.dirty && !self.write_through {
                ...
                self.device
                    .write_block(evicted_block_num, &*evicted.data)
                    .await?;
            }
        }
...
mod tests {
    ...
    async fn test_cache_read_write() {
        ...
        let device = Arc::new(
            FileBackedBlockDevice::create(&path, 2 * BLOCK_SIZE as u64)
                .await
                .unwrap(),
        );
    ...
    async fn test_cache_eviction() {
        ...
        let test_data = [[0x11; BLOCK_SIZE], [0x22; BLOCK_SIZE], [0x33; BLOCK_SIZE]];
