...
use crate::format::{Inode as DiskInode, Superblock, DirEntry, FormatError};
...
pub trait DiskFsTrait: Send + Sync {
    ...
    async fn open(device: Arc<dyn BlockDevice>) -> Result<Self, FsError> where Self: Sized;
    
    /// Format a new filesystem on the given block device
    async fn format(device: &mut dyn BlockDevice, size: u64, volume_name: Option<&str>) -> Result<(), FsError>;
    ...
    async fn read_file_data(&self, inode: &DiskInode, offset: u64, size: u32) -> Result<Vec<u8>, FsError>;
    
    /// Write data to a file's data blocks
    async fn write_file_data(&mut self, inode: &mut DiskInode, offset: u64, data: &[u8]) -> Result<(), FsError>;
...
impl DiskFs {
    ...
    fn new(device: Arc<dyn BlockDevice>, cache: BlockCache, layout: Layout, superblock: Superblock) -> Self {
        Self {
            device,
            cache,
            layout,
...
impl DiskFsTrait for DiskFs {
    ...
    async fn open(device: Arc<dyn BlockDevice>) -> Result<Self, FsError> where Self: Sized {
        ...
        device.read_block(0, &mut superblock_buf).await.into_fs_error()?;
        
        // Parse superblock
        let mut cursor = Cursor::new(superblock_buf);
        let superblock = Superblock::read_from(&mut cursor)
            .map_err(FsError::Format)?;
    ...
    async fn format(device: &mut dyn BlockDevice, size: u64, volume_name: Option<&str>) -> Result<(), FsError> {
        ...
        let mut superblock = Superblock::new(size, volume_name)
            .map_err(|e| FsError::Format(FormatError::Io(e)))?;
        ...
        superblock.write_to(&mut std::io::Cursor::new(&mut superblock_buf))
            .map_err(|e| FsError::Io(BlockDeviceError::Io(e)))?;
            
        device.write_block(0, &superblock_buf).await
        ...
        root_inode.write_to(&mut std::io::Cursor::new(&mut inode_buf))
            .map_err(|e| FsError::Io(BlockDeviceError::Io(e)))?;
            
        // Write root inode to inode table
        device.write_block(layout.inode_block(1).0, &inode_buf).await
            .map_err(FsError::from)?;
        ...
        dot.write_to(&mut cursor)
            .map_err(|e| FsError::Io(BlockDeviceError::Io(e)))?;
        dot_dot.write_to(&mut cursor)
            .map_err(|e| FsError::Io(BlockDeviceError::Io(e)))?;
            
        // Write directory block
        device.write_block(layout.data_block(0), &dir_block).await
            .map_err(FsError::from)?;
    ...
    async fn read_inode(&self, inode_num: u64) -> Result<DiskInode, FsError> {
        ...
        self.cache.read_block(block_num, &mut block_data).await
            .map_err(FsError::Io)?;
    ...
    async fn write_inode(&mut self, inode_num: u64, inode: &DiskInode) -> Result<(), FsError> {
        ...
        self.device.read_block(block_num, &mut block).await.into_fs_error()?;
        ...
        self.device.write_block(block_num, &block).await.into_fs_error()?;
    ...
    async fn read_file_data(&self, inode: &DiskInode, offset: u64, size: u32) -> Result<Vec<u8>, FsError> {
        ...
        while remaining > 0 && current_offset < inode.size {
            ...
            // Read the data block
            let mut block_data = vec![0u8; BLOCK_SIZE];
            self.cache.read_block(self.layout.data_block(block_num), &mut block_data).await
                .map_err(FsError::Io)?;
    ...
    async fn write_file_data(&mut self, inode: &mut DiskInode, offset: u64, data: &[u8]) -> Result<(), FsError> {
        ...
        while remaining > 0 {
            ...
            // For now, we only support direct blocks (first 12 entries in block array)
            if block_idx >= 12 {
                return Err(FsError::InvalidArgument("File too large for direct blocks".to_string()));
            }
            ...
            // Write the block back
            self.device.write_block(self.layout.data_block(block_num), &block_data).await
                .into_fs_error()?;
...
mod tests {
    ...
    async fn test_disk_fs_format() {
        ...
        DiskFs::format(&mut device, 16 * 1024 * 1024, Some("testfs")).await.unwrap();
        ...
        let superblock = Superblock::read_from(&mut std::io::Cursor::new(&superblock_buf[..])).unwrap();
