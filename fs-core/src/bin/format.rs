...
fn main() -> anyhow::Result<()> {
    ...
    let device_size = std::process::Command::new("blockdev")
        ...
        .output()
        .with_context(|| format!("Failed to get device size using blockdev: {:?}", args.device))?;
    ...
    let partition_size = if args.size == 0 {
        info!("Using exact device size: {} bytes ({:.2} GiB)", device_size, device_size_gib);
        device_size
    } else {
        // If size is specified, use it but ensure it doesn't exceed device size
        // Calculate requested size in GiB (2^30 bytes)
        let requested_size = args.size.checked_mul(gibibyte)
            .ok_or_else(|| anyhow!("Requested size is too large"))?;
        
        info!("Device size: {} bytes ({:.2} GiB)", device_size, device_size_gib);
        info!("Requested size: {} bytes ({} GiB)", requested_size, args.size);
        ...
        if requested_size > device_size {
            if requested_size - device_size <= TOLERANCE {
                info!("Device is slightly smaller than requested ({} bytes), using full device size", 
    ...
    if !args.force {
        println!("WARNING: This will format {} as an AegisFS partition ({} GB).", 
                 args.device.display(), args.size);
    ...
    // Format the device
    info!("Formatting {} as AegisFS ({} GB)...", args.device.display(), args.size);
