...
pub mod prelude {
    ...
    pub use crate::modules::{JournalManager, JournalConfig, JournalEntryType};
}
...
impl AegisFS {
    ...
    pub async fn from_device<P: AsRef<Path>>(device_path: P) -> Result<Self> {
        ...
        fs.init_root_cache().await.map_err(|e| Error::Other(format!("Failed to init root cache: {:?}", e)))?;
    ...
    fn get_cached_inode(&self, ino: u64) -> Option<CachedInode> {
        ...
        log::debug!("get_cached_inode: inode {} not in cache, skipping disk load for now", ino);
    ...
    fn write_file_data(&self, ino: u64, offset: u64, data: &[u8]) -> Result<u32> {
        ...
        log::debug!("WRITE: Updated file size to {} bytes for inode {}", new_size, ino);
    ...
    fn read_file_data(&self, ino: u64, offset: u64, size: u32) -> Result<Vec<u8>> {
        ...
        log::debug!("READ: Returning empty data for inode {} (offset={}, size={})", ino, offset, size);
        
        // Return empty data for now
        let data_size = std::cmp::min(size as u64, cached.attr.size.saturating_sub(offset)) as usize;
...
impl Filesystem for AegisFS {
    fn lookup(&mut self, _req: &Request<'_>, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        if let Some(parent_cached) = self.get_cached_inode(parent) {
            ...
            if let Some(&child_ino) = parent_cached.children.get(name_str) {
                log::debug!("LOOKUP: found child '{}' with inode {}", name_str, child_ino);
                if let Some(child_cached) = self.get_cached_inode(child_ino) {
                    reply.entry(&TTL, &child_cached.attr, 0);
                    return;
                }
            } else {
                log::debug!("LOOKUP: child '{}' not found in parent {}", name_str, parent);
            }
        } else {
    ...
    fn create(
        ...
        match self.create_file(parent, name_str, FileType::RegularFile) {
            Ok(cached) => {
                log::debug!("CREATE: successfully created file '{}' with inode {}", name_str, cached.ino);
    ...
    fn mkdir(
        ...
        match self.create_file(parent, name_str, FileType::Directory) {
            Ok(mut cached) => {
                log::debug!("MKDIR: successfully created directory '{}' with inode {}", name_str, cached.ino);
                // Add . and .. entries
    ...
    fn unlink(
        ...
        let (child_ino, is_directory) = {
            ...
            if let Some(parent_cached) = cache.get(&parent) {
                ...
                if let Some(&child_ino) = parent_cached.children.get(name_str) {
                    let is_directory = cache.get(&child_ino)
    ...
    fn rmdir(
        ...
        let (child_ino, is_directory, is_empty) = {
            ...
            if let Some(parent_cached) = cache.get(&parent) {
                ...
                if let Some(&child_ino) = parent_cached.children.get(name_str) {
                    let (is_directory, is_empty) = cache.get(&child_ino)
    ...
    fn rename(
        ...
        if let Some(dest_parent) = cache.get_mut(&newparent) {
            dest_parent.children.insert(newname_str.to_string(), src_ino);
    ...
    fn new_mock() -> Self {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                let mut device = FileBackedBlockDevice::create("/tmp/mock_device", 4 * 1024 * 1024).await.unwrap();
                // Format the device first
                DiskFs::format(&mut device, 4 * 1024 * 1024, Some("MockFS")).await.unwrap();
